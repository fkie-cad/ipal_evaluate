...
def run_command(
    ...
    :param cwd: Optional working directory. Defaults to two levels up from the current script if not provided.
    ...
    if cwd is None:
        ...
        cwd = current_file_path.parent.parent
...
def evaluate(args: List[str]) -> Tuple[int, bytes, bytes]:
    return run_command(EVALUATE, args)


def add_attacks(args: List[str]) -> Tuple[int, bytes, bytes]:
    return run_command(ADD_ATTACKS, args)


def tune(args: List[str]) -> Tuple[int, bytes, bytes]:
    return run_command(TUNE, args)


def csv_to_ipal(args: List[str], stdin: bytes) -> Tuple[int, bytes, bytes]:
    return run_command(CSVTOIPAL, args, stdin=stdin)
